...
class Draft extends Component {
  ...
  componentDidMount() {
    ...
    if(Object.keys(this.props.draftObject).length === 0 && this.props.draftObject.constructor === Object) {
      this.addImageToCanvas('images/ShortTShirtAdultsTemplate.png', 957, 940, 0.6, false, 0, 0); //bg image  
    } else {
      this.canvas.clear().renderAll();
      this.canvas.loadFromJSON(this.props.draftObject);
    }

    this.onDeleteKey();
    //load up new template or sessions' drawing
  }

  componentDidUpdate() {
    if(Object.keys(this.props.draftObject).length === 0 && this.props.draftObject.constructor === Object) {
      this.addImageToCanvas('images/ShortTShirtAdultsTemplate.png', 957, 940, 0.6, false, 0, 0); //bg image  
    } else if(this.state.isDraft === false) {
...
const mapStateToProps = (state) => {
   return {
      draftObject: state.draftObject
   };
};

export default connect(mapStateToProps, { canvasChanged, canvasDone })(Draft);